...
- ~~No wallet integration. No Lace, no signing, no transaction.~~ **Partly stale — and the remaining part must be stated carefully.** The transactions are real: they are signed, proved, pay DUST and land in blocks. But the *service's own* wallet does that, not the browser's. `web/src/wallet.ts` implements the DApp Connector v4.0.1 surface and `onchain/src/chain.ts` exposes `prepare`/`confirm` so a browser wallet can balance a call — that path has now been exercised against an installed wallet (1AM), and it gets four of five steps: connect, read balances, receive a proved transaction, balance it. The wallet's **submission** was refused by the node with `Custom error: 182`, a replay-protection code.
  ...
  Why, measured: the wallet gives its own fee intent about thirty seconds, preview takes 30–118 seconds to include a transaction, so it expires in flight. The contract call beside it carries an hour. The identical prepared transaction is accepted when this project's wallet balances it (tx `00107806…`).

  So the honest phrasing is "the browser path runs to submission, where one wallet's fee intent expires before this network can include it" — not "wallet integration works", not "it has never been tried", and not "we don't know why". The remaining gap is real and it is in the extension, which is worth saying plainly rather than either claiming the feature or hiding behind the ambiguity.
