...
- ~~No wallet integration. No Lace, no signing, no transaction.~~ **Partly stale — and the remaining part must be stated carefully.** The transactions are real: they are signed, proved, pay DUST and land in blocks. But the *service's own* wallet does that, not the browser's. `web/src/wallet.ts` implements the DApp Connector v4.0.1 surface and `onchain/src/chain.ts` exposes `prepare`/`confirm` so a browser wallet can balance a call — **that path has never been exercised against an installed Lace extension**, because no browser wallet was available in the environment where this was built. It is code written to a published spec, not a demonstrated integration, and it should be described that way until someone runs it.
