...
12. **The reference contract targets a pre-release protocol version and cannot be deployed.** Compiler 0.34.0 → `compact-runtime` 0.19.0 → `onchain-runtime-v4 4.0.0-rc.3`. Every live network runs on-chain runtime 3.0.0, and the stable proof server is 8.1.0 against 9.0.0-rc.7 for v4. This was a deliberate trade: the deployable compiler (0.31.1 / language 0.23) has no in-circuit signature verification of any kind, so staying deployable meant giving up the signature. Full trace in RESEARCH.md §F.7c.

    *Correction to an earlier version of this list:* it also claimed `compact-runtime` 0.16.0 exports no signing primitives, and that the alternative was therefore hand-rolling Jubjub Schnorr on both sides. That is wrong. 0.16.0 exports `sampleSigningKey`, `signData`, `signatureVerifyingKey`, `signingKeyFromBip340` and `verifySignature` — the blocker is the *compiler*, not the runtime, and it is one-sided: a signature could be produced in TypeScript but not checked inside a 0.23 circuit, which is the half that matters. See RESEARCH.md §G.2.

12b. **The deployable port trades the signature for a capability secret.** `onchain/contract/credential.compact` authorises an issuer by proof of knowledge of a secret whose digest sits in ledger state. This is a weaker security argument and it is not a smaller one to state: there is no transferable attestation, nothing a third party can verify offline, and anyone who learns the secret can issue and revoke. I6 and I7 survive as secret-knowledge tests rather than forgery tests.
13. **Block-time units are seconds.** No longer an assumption — `BlockContext.secondsSinceEpoch` in `compact-runtime/dist/circuit-context.js:47`, and confirmed on chain: the same `present()` was rejected with `asOf is in the future` when sent in milliseconds and accepted when sent in seconds. RESEARCH.md §H.11.

    *Correction to an earlier version of this list:* it recorded milliseconds as the assumption, and the code matched that assumption throughout. It was wrong, and the way it stayed wrong is worth stating: the simulator supplies block time to itself, so milliseconds-in compares against milliseconds-in and 54 tests pass. **A self-consistent simulator cannot detect a unit error at its own boundary.** The consequence was not merely cosmetic — `freshnessWindow()` returned `300000`, which a chain reading seconds interprets as 3.5 days rather than 5 minutes. The first deployment carried that window; it is preserved under `onchain/deployments/superseded/` rather than deleted.

14. **The default demo mode generates no ZK proofs.** In simulator mode circuits execute and every constraint is enforced, but the prover is not invoked. The on-chain mode is different: it produces real proofs, pays real DUST and waits for real block inclusion. Proving latency is now measured rather than unknown — see the figures recorded by `npm run lifecycle`.
