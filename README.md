...
- **A browser wallet signs the contract calls end to end.** The DApp Connector path has been run against an installed wallet (1AM) and gets four of five steps: the wallet connects, reports its balances, is handed a proved transaction, and balances it. Its submission was refused by the node with `Custom error: 182`, and the cause is now measured rather than suspected: **the wallet gives its own DUST fee intent about thirty seconds to live, and preview takes 30–118 seconds to include a transaction**, so the intent expires before the node validates it. The contract call in the same transaction carries an hour. Full evidence in [RESEARCH.md §H.13a](RESEARCH.md). Nothing here can change it — the expiring intent is built inside the extension — and the same prepared transaction is accepted when this project's own wallet balances it (tx `00107806…`). Until a wallet's TTL outlives a preview block, the service's own wallet is what pays and signs
